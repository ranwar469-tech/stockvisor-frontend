  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-slate-400 mb-8">Track and manage your investments</p>

        {/* Portfolio Summary */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-slate-800 to-slate-700 p-6 rounded-lg border border-blue-600 shadow-lg">
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-700 p-6 rounded-lg border border-blue-600 shadow-lg">
            <p className="text-slate-300 text-sm">Total Gain/Loss</p>
            <p className={`text-3xl font-bold mt-2 ${totalGain >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {totalGain >= 0 ? '+' : ''} ${totalGain.toFixed(2)}
            </p>
          </div>
          <div className="bg-gradient-to-br from-slate-800 to-slate-700 p-6 rounded-lg border border-blue-600 shadow-lg">
            <p className="text-slate-300 text-sm">Return on Investment</p>
        ...
        <div className="bg-slate-800 rounded-lg border border-blue-600 shadow-lg overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-blue-600 bg-slate-700">
                  <th className="text-left p-4 text-slate-300 font-semibold">Symbol</th>
                  <th className="text-right p-4 text-slate-300 font-semibold">Shares</th>
                  <th className="text-right p-4 text-slate-300 font-semibold">Avg Cost</th>
                  <th className="text-right p-4 text-slate-300 font-semibold">Current Price</th>
                  <th className="text-right p-4 text-slate-300 font-semibold">Gain/Loss</th>
                  <th className="text-right p-4 text-slate-300 font-semibold">Value</th>
                </tr>
              </thead>
              <tbody>
                {holdings.map((holding) => (
                  <tr key={holding.symbol} className="border-b border-slate-700 hover:bg-slate-700 transition-colors">
                    <td className="p-4 font-bold text-blue-400">{holding.symbol}</td>
                    <td className="text-right p-4 text-slate-300">{holding.shares}</td>
                    <td className="text-right p-4 text-slate-300">${holding.avgCost.toFixed(2)}</td>
                    <td className="text-right p-4 font-semibold text-white">${holding.current.toFixed(2)}</td>
                    ...
                    <td className="text-right p-4 font-semibold text-white">${(holding.shares * holding.current).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
